...
if ($dopost == 'bak') {
    ...
    if ($nowtable == '') {
        ...
        if ($isstruct == 1) {
            ...
            $bkfile = $bkdir . "/tables_struct--1--{$time}--".substr(md5($time . mt_rand(1000, 5000) . "K2QJLQJGD107TM9APR811IELBO"), 0, 8).".txt";
            $fp = fopen($bkfile, "w");
    ...
    //执行分页备份
        ...
        $dsql->Execute();
        $m = 0;
        $bakfilename = "$bkdir/{$nowtable}--{$startpos}--{$time}--".substr(md5($time . mt_rand(1000, 5000) . "K2QJLQJGD107TM9APR811IELBO"), 0, 8).".txt";
